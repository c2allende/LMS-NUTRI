...
| Archivo | Descripción |
|---|---|
| `dashboard.html` | Dashboard del estudiante — banner de notificaciones EC condicional (3 niveles: vencida/urgente/rezagado), dismiss por sessionStorage; sidebar incluye enlace "Mi Progreso" → `dashboard_analytics.html` |
...
| `admin_evaluaciones.html` | Resultados globales de evaluaciones — 4 KPIs, stats por oferta, tabla filtrable (búsqueda, oferta, resultado, período), mock data de demostración |
| `admin_ofertas.html` | Catálogo de ofertas EC — 3 KPIs, grid de 5 ofertas con stats de Firestore, modal placeholder para edición |
| `admin_comunicaciones.html` | Centro de mensajería admin → estudiantes — formulario (destinatarios, tipo, asunto, mensaje), historial desde Firestore `comunicaciones/`, mock de 4 entradas, nota sobre Firebase Functions |
| `admin_reportes.html` | Analytics agregados — 4 KPIs globales, gráfica SVG de horas EC por mes (últimos 6 meses), ranking de ofertas por tasa aprobación, distribución por tipo, exportación placeholder |
| `dashboard_analytics.html` | Analytics personal del estudiante — 4 KPIs (horas EC + barra meta, ofertas completadas, score promedio, racha), gráfica SVG 6 meses, progreso por oferta, proyección de meta, historial de evaluaciones |
| `admin_usuarios.html` | Gestión de usuarios (legado) |
| `admin_anuncios.html` | Gestión de anuncios (legado — reemplazado por admin_comunicaciones.html) |
...
| `catalog-data.js` | externo | Array `CATALOG_OFFERINGS` (5 ofertas E1–E5) + helpers; cada oferta incluye `offerTitle` (clave Firestore) e `introUrl` |
...
| `admin-evaluaciones-controller.js` | externo | `collectionGroup('quiz_results')` — KPIs globales, stats por oferta, tabla filtrable; MOCK_RESULTS de 8 entradas cuando Firestore vacío |
| `admin-ofertas-controller.js` | externo | KPIs desde `CATALOG_OFFERINGS`, grid de ofertas enriquecido con Firestore (`offerTitle` como clave), `introUrl` dinámico, modal placeholder |
| `admin-comunicaciones-controller.js` | externo | CRUD de `comunicaciones/` — sendCommunication (escribe Firestore), loadCommunications (fallback a MOCK de 4 entradas), historial con filas expandibles, toast, select de oferta dinámico |
| `admin-reportes-controller.js` | externo | `Promise.all` sobre `usuarios`, `certificados_plataforma` y `quiz_results` — KPIs, gráfica SVG de barras (6 meses), ranking de ofertas, distribución por tipo desde CATALOG_OFFERINGS, modal exportación placeholder |
| `analytics-controller.js` | externo | Analytics personal del estudiante — `Promise.all` sobre `certificados_plataforma`, `certificados_externos`, `quiz_results`, `usuarios/{uid}`; SVG de barras (6 meses) con línea de meta 1.5 h; progreso por oferta con sessionStorage; proyección de meta EC con alerta de vencimiento |
| `ec-notifications-controller.js` | externo | Notificaciones de vencimiento EC en `dashboard.html` — `Promise.all` sobre `usuarios/{uid}`, `certificados_plataforma`, `certificados_externos`; 3 tipos: `expired` (licencia vencida), `urgent` (≤90 días + <36h), `behind` (≤180 días + <18h); dismiss por sessionStorage; `textContent` para datos de Firestore |
